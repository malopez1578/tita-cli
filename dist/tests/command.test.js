...
const command_1 = require("../src/utils/command");
describe('CommandExecutor', () => {
    ...
    beforeEach(() => {
        ...
        commandExecutor = new command_1.CommandExecutor(mockLogger);
        jest.clearAllMocks();
    });
    describe('constructor', () => {
        it('should create an instance with logger', () => {
            expect(commandExecutor).toBeInstanceOf(command_1.CommandExecutor);
        });
    });
    describe('error handling', () => {
        it('should create proper error types', () => {
            const cmdError = new errors_1.CommandExecutionError('test cmd', 1);
            expect(cmdError).toBeInstanceOf(errors_1.CommandExecutionError);
            expect(cmdError.message).toContain('Command execution failed: test cmd');
            expect(cmdError.code).toBe('COMMAND_EXECUTION_ERROR');
            const prereqError = new errors_1.PrerequisiteError('git');
            expect(prereqError).toBeInstanceOf(errors_1.PrerequisiteError);
